    ...
    <div class="container mx-auto px-4 lg:px-0">
      <div class="block lg:flex flex-wrap justify-between">
        <div class="w-full lg:w-half p-10 text-secondaryColor bg-mainColor rounded-tl-xl mb-2">
          <router-link :to="{ name: 'SectorPage', params: { id: 4 } }" v-if="activeBlock === null">
            <h3 class="mb-2 text-2xl font-bold">
              {{ $t('mainPage.indSectors.title') }}
            </h3>
            <p class="h-40 max-h-40 overflow-y-auto">
              {{ $t('mainPage.indSectors.content') }}
            </p>
          </router-link>
          <div class="opacity-0 transition-all" :class="{ '!opacity-100' : getIndSectors[activeBlock] }" v-else>
            ...
            <h3 v-if="$i18n.locale === 'kz'" class="mb-2 text-2xl font-bold">{{
                getIndSectors[activeBlock].title_kz
              }}</h3>
            <h3 v-if="$i18n.locale === 'en'" class="mb-2 text-2xl font-bold">{{
                getIndSectors[activeBlock].title_en
              }}</h3>
            <p class="h-40 max-h-40 overflow-y-auto" v-if="$i18n.locale === 'ru'">
              {{ getIndSectors[activeBlock].description }}</p>
            <p class="h-40 max-h-40 overflow-y-auto" v-if="$i18n.locale === 'kz'">
              {{ getIndSectors[activeBlock].description_kz }}</p>
            <p class="h-40 max-h-40 overflow-y-auto" v-if="$i18n.locale === 'en'">
              {{ getIndSectors[activeBlock].description_en }}</p>
          </div>
        </div>
        <router-link
            ...
            :class="[{ '!grayscale-0' : activeBlock === index }, { 'rounded-tr-xl' : index === 0 }, { 'rounded-bl-xl' : index === 1 }, { 'rounded-br-xl' : index === 2 }, { 'lg:w-third' : index === 1 || index === 2 || index === 3 }]"
            v-for="(item, index) of getIndSectors" :key="index">
          <img class="w-full" :class="[{ 'rounded-tr-xl' : index === 0 }]" :src="item.img" alt="">
          <p v-if="activeBlock === index && $i18n.locale === 'ru'"
             class="absolute left-8 bottom-7 z-20 text-lg lg:text-2xl text-white font-bold">{{ item.title }}</p>
          <p v-if="activeBlock === index && $i18n.locale === 'kz'"
             class="absolute left-8 bottom-7 z-20 text-lg lg:text-2xl text-white font-bold">{{ item.title_kz }}</p>
          <p v-if="activeBlock === index && $i18n.locale === 'en'"
             class="absolute left-8 bottom-7 z-20 text-lg lg:text-2xl text-white font-bold">{{ item.title_en }}</p>
          <div
